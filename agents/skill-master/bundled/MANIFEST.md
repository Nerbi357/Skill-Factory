...
Философия держит принципы, зоны, закон и карту связей — то, с чем агент сверяет
библиотеку; ворота, пять тестов и анатомию артефакта держит скилл `skill-anatomy`,
и работа CREATE читает его от корня репозитория. Без философии агент может
прочитать библиотеку, но ему не с чем её сверять.
