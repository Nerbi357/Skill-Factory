...
**План в нём ведётся фазами.** Фаза — блок работы со своей целью, и в файле она
состоит из трёх вещей: цель, принятые решения, шаги. Цель называет конкретную
работу и её результат, а не описывает состояние и не звучит лозунгом. Каждое
решение несёт краткое
«почему» — решение без рассуждения можно только исполнить или проигнорировать, но
не применить к случаю, которого никто не предвидел. Шаги есть только у незакрытых
фаз: закрытая теряет их и оставляет одни решения, и это единственное, чем запись о
прошлом отличается от записи о настоящем. Закрытая фаза не переписывается — если
позже что-то делается заново, это новая фаза, которая говорит, что изменилось и
почему.

В нём же живёт **рабочее соглашение** — как автор хочет, чтобы с ним работали:
что показывать, о чём спрашивать до действия, сколько подробностей, в какой форме
отвечать, чего он видеть не хочет. Оно лежит здесь, а не в скиллах, потому что
меняется от проекта к проекту и от чата к чату, а скилл меняется куда медленнее.
Тест принадлежности: запись меняет то, *как об этом говорят*. Если она меняет то,
*что́ считается сделанным правильно*, — это метод, и он входит в скилл.
...
**`PROJECT_PHILOSOPHY.md` — принципы, по которым репозиторий работает.** Как
устроен сам репозиторий — ветки, папки, файлы, что где лежит и почему. Сюда же
идут анти-паттерны и особенности работы, не выделенные в отдельные скиллы. Как
устроено взаимодействие с автором, здесь не пишут: рабочее соглашение живёт в
`PROJECT_MEMORY.md`, и причина названа выше.

**В нём только принципы, и он читается отдельно от всех остальных файлов.** Ни
ссылки на другой репозиторий, ни ссылки на скилл, ни на агента, ни на соседний
документ: ничего из этого не должно требоваться, чтобы файл был понятен целиком.
Рабочие и стартовые промпты, технические цифры и инструкции агенту живут в
`PROJECT_MEMORY.md` — здесь они превращают конституцию в инструкцию, и первое же
изменение платформы делает её неверной. Исключение одно и названо ниже: порядок
поддержания самого проекта пишется здесь, потому что он адресован автору, а не
агенту, и меняется вместе с устройством проекта, а не с платформой.

В нём обязательны два отдельных блока.

**«How to maintain»** — для автора проекта: что он делает, чтобы проект
обновлялся и оставался работоспособным. Не процедура для агента, а порядок,
которого держится сам автор: что перечитывает, что запускает, чем закрывает
работу, что проверяет перед тем, как считать проект живым. **Любое идейное
изменение, затрагивающее проект целиком, отражается в этом файле.** Если изменение
не попало сюда, оно потеряется при следующей смене сессии.

**«Как части соединены»** — единственное место, где называются связи и
зависимости между файлами и папками: кто что пишет, кто что читает, что из чего
генерируется, что производно от чего и какое правило эту связь охраняет. Он и
есть вторая половина запрета выше: сначала связи выселяются отовсюду, потом
получают один адрес. Если связей в проекте нет — так и напиши одной строкой.
