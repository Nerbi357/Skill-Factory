---
name: signal-capture
description: Записывает наблюдения о том, как идёт работа, в файл SIGNALS.md, чтобы позже они улучшили библиотеку скиллов, а не забылись. Применяй в каждом проекте, постоянно. Применяй в момент, когда владелец тебя поправил, когда что-то стоило больше сообщений, чем должно было, когда ты объясняешь одно и то же дважды, когда пришла задача, которую не покрывает ни один скилл, или когда твоё утверждение оказалось неверным. Применяй, подводя черту под работой, чтобы предложить короткий разбор.
---
...
**Оставляет за скобками:** «что из записанного должно что-то изменить» — это
суждение разбора, где решает владелец: написать сигнал не значит предложить
починку.
...
**Наблюдение владельца о том, как идёт работа, старше твоего.** Он сказал, что
что-то легло плохо, — это сигнал высшего доступного качества: записывай его
дословно и в тот же момент, не пропуская через своё суждение о том, стоит ли оно
записи. Твои собственные наблюдения проходят проверку из раздела «Когда писать»;
его — нет.
...
**Предложение нового скилла или агента — тоже `gap`.** Владелец говорит, чего не
хватает, и называет, чем эта вещь должна владеть; записывай это его рассуждением
и видом `gap`. Действовать по такой записи нельзя: новый артефакт заводит разбор,
а запись — только материал для него.
...
**На номер не ссылаются за пределами журнала.** Он говорит, где запись лежит, и
после очистки будет указывать на другую; ссылка из чужого файла сломается молча.
Нужно сослаться — назови случай, а не номер.
...
Формат потяжелее не используется: ненаписанный сигнал не стоит ничего, как бы
хорошо он ни был оформлен. Объём платят в «Что случилось» и «Дословно», а не в
новых полях.
...
- **естественная граница работы** — этап закончен, черта подведена
- **конец проекта**
...
Если разбор нужен, он происходит в библиотеке скиллов, а не здесь.
...
мебелью. Конец этапа — естественный момент; конец мелкой задачи — нет.
...
— в разговоре, где вердикт приняли, и в коммите, который его применил. Всё
неразобранное оставь ровно как есть.
