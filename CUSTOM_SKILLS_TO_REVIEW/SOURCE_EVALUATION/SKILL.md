...
`CONFIDENCE_CHECK`, which this skill leans on throughout. Also how to build the
...
`CUSTOM_AGENTS_TO_REVIEW/SOURCE_SCOUT/` for the same verdict.
...
- **(A)** Keep both: this skill holds the method, `SOURCE_SCOUT` reads it and adds
...
`CONFIDENCE_CHECK` as unrelated to how well a fact is known, and it appears here as
