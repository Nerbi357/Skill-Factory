...
PAIRS = {
    ...
    # «измерение несёт свою дату» — и оговорка про образец формы: цифра внутри
    # примера ни о чём не заявляет и даты не требует. Адресата три, и каждый
    # спрашивает своё: `confidence-check` — насколько известен факт,
    # `git-repo-structure` — какая проза перестаёт быть правдой, чек-лист —
    # что проверяют перед тем, как назвать работу законченной.
    "измерение несёт свою дату": [
        ("skills/confidence-check/SKILL.md", "Измерение — это факт об одном моменте."),
        ("skills/confidence-check/SKILL.md", "Правило судит заявленный факт, а не образец формы."),
        ("skills/git-repo-structure/SKILL.md", "до тех пор, пока несёт дату или прогон, из которого взялось"),
        ("skills/git-repo-structure/references/FINAL_PASS.md", "Каждое измерение, заявленное как факт, несёт свою дату или прогон."),
        ("skills/git-repo-structure/references/FINAL_PASS.md", "Образец формулировки фактом не является и даты не требует."),
    ],
    ...
    # «три хода» — что делают со скиллом, переставшим окупаться, и в каком
    # порядке. Содержание держит философия: проход «Мёртвое» работы REVIEW
    # отсылает к ней словами и своей редакции не несёт, поэтому переписанный
    # раздел философии оставил бы проход без содержания молча.
    "три хода": [
        ("PROJECT_PHILOSOPHY.md", "Два слабых скилла, покрывающих соседнюю землю, становятся одним сильным."),
        ("PROJECT_PHILOSOPHY.md", "Удаление не входит ни в один из них и никогда не происходит без слова владельца."),
        ("skills/skill-anatomy/SKILL.md", "Скилл, переставший окупаться, не удаляется — он сливается, переписывается или, только с явного разрешения владельца, отправляется на покой."),
        ("agents/skill-master/AGENT.md", "Никогда не предлагай удаление."),
        ("agents/skill-master/JUDGEMENT.md", "Удаление скилла вместо слияния или переписывания"),
        ("agents/skill-master/README.md", "первым он предложит слить два слабых скилла в один сильный"),
        ("PROJECT_MEMORY.md", "отправку артефакта на покой, которая вдобавок требует его прямого слова"),
    ],
}
