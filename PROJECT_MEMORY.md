...
- **Взгляд со стороны** — намеренно свежая сессия без рабочего контекста,
  ...
  и есть смысл — это тест на состязательную независимость из философии. Её промпт
  тоже ниже.
...
После решения они записываются под фазой ниже и не переоткрываются посреди
работы; если реальность противоречит решению, рабочая сессия говорит об этом и
переспрашивает, а не отклоняется молча.

**Pull request'ы.** Одно осмысленное изменение на PR; механическая возня —
отдельными коммитами от смысла; заголовки коммитов по скиллу
`git-repo-structure`. Потолок
...
Как владелец хочет, чтобы с ним работали: что показывать, о чём спрашивать до
действия, сколько подробностей, в какой форме отвечать, чего он видеть не хочет.
Раздел живёт здесь, а не в скиллах, потому что меняется от проекта к проекту и от
чата к чату, а скилл меняется куда медленнее.

**Тест, что запись принадлежит сюда, а не скиллу:** она меняет то, *как об этом
говорят*. Если она меняет то, *что́ считается сделанным правильно*, — это метод, и
он входит в скилл, которому принадлежит.

Раздел меняется только по собственным сигналам владельца. Майнинг его не трогает
никогда.

- **Отвечать структурно.** Таблицы, пронумерованные разделы, явные варианты — не
  сплошная проза. Каждая идея помечена тем, куда пойдёт и что заденет. Дословно,
  2026-08-04: «мне не нравится формат твоего ответа: я просил, чтобы он был
  структурный, четкий... а получился просто большой текст без структуры и с
  непонятными вопросами».
- **Вопросы задаются сразу, в том же сообщении**, и каждый несёт варианты с
  рекомендацией, а не задаётся голым. Дословно, 2026-08-09: «если у тебя есть
  вопросы или сомнения, смело задавай их мне, никогда не принимай идейные
  решения, если не уверен в них или из-за недосказанности, все проясняй».
- **Ничего не пишется до согласия.** Правка вносится только после того, как
  обсуждено и одобрено, что именно будет отредактировано. Дословно, 2026-08-09:
  «делай PR сам, но только после моего окончательного одобрения того, что именно
  будешь редактировать».
- **Быть максимально критичным.** Предлагать другие варианты, говорить, что можно
  улучшить и расширить, об ошибках говорить явно. Дословно, 2026-08-10: «будь
  максимально критичен, предлагай другие варианты, что можно улучшить, что
  расширить» · «если видишь ошибки, явно говори о них».
- **Отчёт — список на одобрение, а не рецензия.** Дословно, 2026-08-04: «чтобы я
  мог отсмотреть все полезные блоки/идеи и дать одобрение на включение в тот или
  иной файл».
...
— это работа шага о рабочих скиллах.*
...
- **Команды: припаркованы, 2026-08-04.** Ворота старой Фазы 1 были проведены и
  ...
  `to_review/COMMANDS.md` как спецификация, а вопрос возвращается в последней фазе. План
  ниже перенумерован соответственно — **Фаза 1 теперь база**.
- **Собственные механизмы фабрики включены, 2026-08-04.** `skill-master`
...
- **Пересмотр основных файлов, с 2026-08-09.** Развитие приостановлено; репозиторий
  временно переведён на русский, чтобы владелец прочитал его целиком, и будет
  возвращён на английский откатом переводных коммитов. Работа идёт в ветке
  `better_look` шагами: философия, память, формат идей, `skill-master`, рабочие
  скиллы, `.claude/`, остальное. Шаг закрывается, только когда его закрывают обе
  стороны.
- **Пометка `_CHECK` в именах, с 2026-08-10.** Файл, прошедший разбор и правку,
  получает суффикс `_CHECK` в имени, чтобы список файлов сам показывал, что уже
  закрыто. Это временное имя рабочей стадии, и конец у него объявлен: **все
  суффиксы снимаются, когда пересмотр закончен**, тем же движением, что и возврат
  на английский. Суффикс ставится только на файлы этого репозитория; в
  `git-repo-structure` и его чек-листе имя `PROJECT_PHILOSOPHY.md` остаётся без
  суффикса, потому что там оно обязательное имя для любого проекта, а не ссылка на
  наш файл. Пока суффикс стоит, корень фабрики своему же скиллу не соответствует —
  это известно и принято на время пересмотра.
- **Долг после шага 1, 2026-08-10.** Четыре скилла всё ещё несут разделы
  «Предпочтения владельца» — шестнадцать строк, — которых конституция больше не
  допускает. Разбираются на шаге о рабочих скиллах: часть уходит в тела скиллов,
  часть в «Рабочее соглашение» выше. До тех пор скиллы отстают от философии, и
  это известно.
- **`verify-before-done` в силе**, 2026-08-04 — первая промайненная папка и
  ...
  `skill-master` его не читает, потому что судит и пишет, а не строит то, что
...
**Как это делается без команд:** метод майнинга — это работа `agents/skill-master`,
...
- **Ничто не вводится в силу целиком.** Заимствованный артефакт разбирается на
  ...
  себе результат.
- **Рабочее соглашение майнинг не трогает никогда.** Оно меняется только по
  собственным сигналам владельца. Это защита от разбавления вкуса, и она
  абсолютна. До 2026-08-10 то же правило защищало разделы предпочтений внутри
  скиллов; разделов больше нет, защищаемое переехало в «Рабочее соглашение» ниже.
- **Слить прежде, чем добавить.** Кусок, расширяющий существующий скилл, расширяет
  его; новой папке нужен довод, почему земля ещё не покрыта, — ворота, за
  которыми скиллу позволено существовать, стоят в философии.
...
1. **`skill-master` её читает** и возвращает разбор — каждое отдельное
...
`PROJECT_PHILOSOPHY_CHECK.md`, предлагается отсюда, но никогда отсюда не мержится. Это
...
по умолчанию: каждый кандидат проходит ворота, за которыми скиллу позволено
существовать, на своих мини-воротах.
...
- **Команды не строятся.** Команда — ручка на работе, которая и так достижима, а
  не способность; семь проектов остаются в `to_review/COMMANDS.md` как спецификация, а
...
- ~~**Два слоя делят механизм и манеру, а не «общее» и «его».**~~ — **отменено
  2026-08-10.** Двух слоёв больше нет: скилл целиком состоит из метода, а
  предпочтения о том, как вести разговор, живут в «Рабочем соглашении» этого
  файла. Причина отмены — не ошибка деления, а то, что верхний слой не должен был
  ехать вместе со скиллом: манера меняется от проекта к проекту, а скилл нет.
  Уцелевшая половина правила осталась принципом: «это его явное указание» — не
  довод внутри скилла.
...
- **Отчёт MINE — это список включений на одобрение**, а не рецензия на качество
  источника. Его форма зафиксирована в `agents/skill-master/AGENT.md`: девять
...
- **Перекрёстная ссылка называет свой файл**, и в артефактах, и в отчётах. Голый
  номер раздела читается только тем, кто уже держит оба текста. С 2026-08-10
  строже: на номер раздела чужого файла не ссылаются вовсе — правило называют
  словами.
- ~~**Перенос правила между двумя слоями — исправление, а не нарушение**~~ —
  **отменено 2026-08-10** вместе с самими слоями. Осталась только его форма
  отчётности: перенос правила между файлами сообщается отдельной пронумерованной
  строкой, никогда сноской.
...
Решено владельцем 2026-08-10, при разборе философии:

- **Скилл целиком состоит из метода.** Раздела предпочтений внутри скилла больше
  нет: то, что меняет, *что́ считается сделанным правильно*, входит в тело скилла
  или в другой скилл; то, что меняет, *как об этом говорят*, живёт в «Рабочем
  соглашении» этого файла.
- **Строка предпочтения, которую нельзя защитить по существу, в тело скилла не
  переезжает.** Либо она уходит в рабочее соглашение, либо не выживает. Правило
  действует на разборе шестнадцати строк и там же служит проверкой самого решения
  выше: если тела скиллов от него укрепятся, деление было верным.
- **Список поверхностей — исходное состояние, а не потолок.** Новое место
  появляется по необходимости и только с разрешения владельца, спрошенного до
  того, как файл написан; вместе с разрешением объявляется, поверхность это или
  служебный файл.
- **Философия читается отдельно от всех остальных файлов.** Вне раздела «Как
  части соединены» она не ссылается ни на репозиторий, ни на скилл, ни на агента,
  ни на другой документ. Правило записано в `git-repo-structure` и распространяется
  на любой проект.
...
| Майнинг разбавляет вкус владельца | правки, задевающие «Рабочее соглашение» | оно меняется только по его собственным сигналам — никогда майнингом |
...
Вставляется в сессию **другого** проекта, чтобы она знала о библиотеке и умела
ею пользоваться. Держится здесь, а не на витрине: это рабочий инструмент, а
витрина не место для инструментов.
...
> `SIGNALS.md` (что не разобрано). Затем скажи одним
...
> сообщении, варианты с рекомендацией. То, что закреплено за мной списком «что она
> кладёт владельцу всегда» (создание, слияние или отправка артефактов на покой;
> изменение философии; перепланировка фаз), — клади мне на воротах, никогда
> посреди задачи. А когда ты
...
- **Цифры среды, измерено 2026-07-28 по документации Claude Code.** Перепроверь,
  прежде чем опираться на любую из них; философия несёт свойства среды без цифр
  именно затем, чтобы изменившийся лимит не отменял принцип. Сжатие переносит
...
- **`AI_INSTRUCTIONS (1).md` и `SKILL (1).md`** лежат в `to_review/` с
  2026-08-07, по слову владельца, и ждут своей очереди на майнинг там же, где
  остальное сырьё. Корень их больше не держит.
